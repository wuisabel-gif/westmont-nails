<?php require_once('includes/head.php') ?>
<?php require_once('includes/header.php') ?>
<main>
    <section class="band">
        <div class="wrap">
            <p class="eyebrow" style="justify-content:center">The work</p>
            <h1>Finished hands &amp; <span class="serif-it">faces.</span></h1>
            <p class="lead">Recent looks from the chair &mdash; gloss, colour, and a little gold where it counts.</p>
        </div>
    </section>

    <section class="section">
        <div class="wrap">
            <!--
              Each tile uses a real image in /images. To use your own photo,
              just save it over the file with the SAME name (e.g. images/cherry-gloss.jpg)
              and the gallery updates automatically. Portrait shots (3:4 or 4:5) look best.
            -->

            <!-- ============ NAILS ============ -->
            <div class="gallery-cat"><h2>Nails</h2><span class="rule"></span></div>
            <div class="gallery-grid">

                <figure class="tile">
                    <img src="images/gilded-french.jpg" alt="Gilded French manicure in nude with gold tips">
                </figure>

                <figure class="tile">
                    <img src="images/rosewood-chrome.jpg" alt="Rosewood chrome nail art">
                </figure>

                <figure class="tile">
                    <img src="images/almond-nude.jpg" alt="Almond nude signature manicure">
                </figure>

                <figure class="tile">
                    <img src="images/cherry-gloss.jpg" alt="Cherry gloss wine gel manicure">
                </figure>

                <figure class="tile">
                    <img src="images/copper-tips.jpg" alt="Copper gradient tip extensions">
                </figure>

                <figure class="tile">
                    <img src="images/pearl-ombre.jpg" alt="Pearl ombre nail art">
                </figure>

                <figure class="tile">
                    <img src="images/midnight-plum.jpg" alt="Midnight plum gel manicure with gold accent">
                </figure>

                <figure class="tile">
                    <img src="images/bridal-lace.jpg" alt="Bridal white nails with lace detail">
                </figure>

                <figure class="tile">
                    <img src="images/soft-lily.jpg" alt="Soft lily lilac pink almond nails">
                </figure>

            </div>

            <!-- ============ FACE & MAKEUP ============ -->
            <div class="gallery-cat"><h2>Face &amp; Makeup</h2><span class="rule"></span></div>
            <div class="gallery-grid">

                <figure class="tile">
                    <img src="images/bridal-glam.jpg?v=2" alt="Soft bridal makeup with glossy nude lip">
                </figure>

                <figure class="tile">
                    <img src="images/soft-glam.jpg" alt="Evening soft-glam makeup with bronze tones and lashes">
                </figure>

                <figure class="tile">
                    <img src="images/fresh-face.jpg?v=2" alt="Natural fresh-faced everyday makeup look">
                </figure>

                <figure class="tile">
                    <img src="images/event.jpg" alt="Glamorous event makeup with smoky eye and glowing skin">
                </figure>

                <figure class="tile">
                    <img src="images/evening-glam.jpg" alt="Evening glam makeup with soft smoky eye and curls">
                </figure>

                <figure class="tile">
                    <img src="images/graduation.jpg" alt="Soft natural graduation-day makeup">
                </figure>

                <figure class="tile">
                    <img src="images/engagement.jpg" alt="Soft romantic engagement-day makeup">
                </figure>

                <figure class="tile">
                    <img src="images/mother-of-bride.jpg" alt="Elegant mother-of-the-bride makeup">
                </figure>

                <figure class="tile">
                    <img src="images/birthday.jpg" alt="Birthday glam makeup look">
                </figure>

            </div>

            <!-- ============ LASHES, BROWS & FACIALS ============ -->
            <div class="gallery-cat"><h2>Lashes, Brows &amp; Facials</h2><span class="rule"></span></div>
            <div class="gallery-grid">

                <figure class="tile">
                    <img src="images/volume-lashes.jpg?v=3" alt="Before and after lash extensions">
                </figure>

                <figure class="tile">
                    <img src="images/tinted-brows.jpg" alt="Groomed, tinted eyebrows with soft glowing makeup">
                </figure>

                <figure class="tile">
                    <img src="images/custom-facial.jpg" alt="Custom facial before and after">
                </figure>

                <figure class="tile">
                    <img src="images/facial-phases.jpg" alt="Custom facial treatment phases">
                </figure>

            </div>

            <div style="text-align:center;margin-top:3rem">
                <a class="btn-lux" href="contact.php">Book your look</a>
            </div>
        </div>
    </section>
</main>
<?php require_once('includes/footer.php') ?>
